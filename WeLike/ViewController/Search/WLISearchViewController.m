...
- (void)reloadData:(BOOL)reloadAll {
    ...
    NSUInteger page = reloadAll ? 1 : (self.users.count / kDefaultPageSize) + 1;
    [sharedConnect usersForSearchString:self.searchBarSearchUsers.text page:(int)page pageSize:kDefaultPageSize onCompletion:^(NSMutableArray *users, ServerResponse serverResponseCode) {
        loading = NO;
        if (reloadAll) {
