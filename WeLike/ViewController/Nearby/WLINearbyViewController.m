...
- (void)viewDidLoad {
    ...
    [reloadButton setImage:[UIImage imageNamed:@"nav-btn-reload"] forState:UIControlStateNormal];
...
- (void)reloadData {
    ...
    [hud show:YES];
    loading = YES;
    NSUInteger page = (self.users.count / kDefaultPageSize) + 1;
    [sharedConnect usersAroundLatitude:lastLocation.coordinate.latitude longitude:lastLocation.coordinate.longitude distance:10000 page:(int)page pageSize:kDefaultPageSize onCompletion:^(NSMutableArray *users, ServerResponse serverResponseCode) {
        [hud hide:YES];
        loading = NO;
        self.users = users;
...
- (MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id < MKAnnotation >)annotation {
    ...
    if (!annotationView) {
        ...
        UIImage *buttonImage = [UIImage imageNamed:@"details-arrow"];
